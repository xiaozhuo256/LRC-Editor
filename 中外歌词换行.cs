...
class 文件交互
{
    ...
    public static void 打开文件夹()
    {
        ...
        DirectoryInfo 打开的目录 = new DirectoryInfo(交互菜单.用户输入_文件夹地址in);

        // 检查目录是否存在
        if (打开的目录.Exists)
        {
            输出.输出正确信息("已找到" + 交互菜单.用户输入_文件夹地址in + "文件夹");
            // 获取文件夹中的文件和子文件夹
            string[] 文件们 = Directory.GetFiles(交互菜单.用户输入_文件夹地址in);
            string[] 子文件夹们 = Directory.GetDirectories(交互菜单.用户输入_文件夹地址in);
            ...
            else
            {
                ...
                文件交互.遍历目录(交互菜单.用户输入_文件夹地址in, 总文件列表, lrc文件列表, flac文件列表, 其他文件列表);
        ...
        else
        {
            ...
            {
                // 创建“lrcin”文件夹
                Directory.CreateDirectory(交互菜单.用户输入_文件夹地址in);
                输出.输出正确信息("已创建“" + 交互菜单.用户输入_文件夹地址in + "”文件夹");
            }
            catch (Exception ex)
            {
                输出.输出错误信息("创建“" + 交互菜单.用户输入_文件夹地址in + "”文件夹时出现错误：" + ex.Message);
            }
        }
    }
